...
def ensure_compile_time_eval():
  ...
  undesirable. For example, if numerical values might be needed to evaluate
  Python control flow and so their evaluation cannot be delayed. As another
  example, it may be beneficial to ensure compile time evaluation (or "constant
  folding") for performance reasons.
  ...
  Here's a real-world example from https://github.com/google/jax/issues/3974::
    ...
    @jax.jit
    def jax_fn(x):
        with jax.core.eval_context():  # temporary name, see #7987
